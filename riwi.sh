...
set_password() {
  clear
  echo -e "${rw_hong}╔════════════════════════════════════════╗${rw_bai}"
  echo -e "${rw_hong}║   ${rw_huang}首次使用 - 设置访问密码${rw_hong}              ║${rw_bai}"
  echo -e "${rw_hong}╚════════════════════════════════════════╝${rw_bai}"
  echo ""
  
  while true; do
    echo -e "${riwi001}请设置访问密码（至少8位，包含大小写字母和数字）${rw_bai}"
    ...
    if [ ${#password1} -lt 8 ]; then
      echo -e "${rw_hong}错误：密码长度至少需要8位${rw_bai}"
      continue
    fi
    
    if ! echo "$password1" | grep -q '[A-Z]' || ! echo "$password1" | grep -q '[a-z]' || ! echo "$password1" | grep -q '[0-9]'; then
      echo -e "${rw_hong}错误：密码需要包含大小写字母和数字${rw_bai}"
      continue
    fi
    
    if [ "$password1" != "$password2" ]; then
      echo -e "${rw_hong}错误：两次输入的密码不一致${rw_bai}"
      continue
    fi
  ...
  echo ""
  echo -e "${rw_lv}✓ 密码设置成功！${rw_bai}"
  echo -e "${rw_huang}请牢记您的密码${rw_bai}"
  echo ""
  sleep 2
}
...
verify_password() {
  ...
  if [ -f "$PASSWORD_TIMESTAMP_FILE" ]; then
    ...
    if [ "$time_diff" -lt "$timeout_seconds" ]; then
      ...
      if [ "$remaining_minutes" -gt 0 ]; then
        echo -e "${rw_lv}✓ 密码已在免密期内（还剩 ${remaining_minutes} 分钟）${rw_bai}"
      else
        echo -e "${rw_lv}✓ 密码已在免密期内${rw_bai}"
      fi
      sleep 1
      return 0
    fi
  ...
  while [ $attempt -le $max_attempts ]; do
    ...
    echo -e "${riwi001}║   ${rw_huang}Riou脚本工具箱 - 身份验证${rw_bai}        ║${rw_bai}"
    echo -e "${riwi001}╚════════════════════════════════════════╝${rw_bai}"
    echo ""
    echo -e "${rw_huang}尝试次数: $attempt / $max_attempts${rw_bai}"
    echo ""
  ...
  # 达到最大尝试次数
  echo -e "${rw_hong}✗ 已达到最大尝试次数，访问被拒绝${rw_bai}"
...
install() {
	...
	for package in "$@"; do
		if ! command -v "$package" &>/dev/null; then
			echo -e "${riwi001}正在安装 $package...${rw_bai}"
...
check_disk_space() {
	...
	if [ "$available_space_mb" -lt "$required_space_mb" ]; then
		echo -e "${rw_huang}提示: ${rw_bai}磁盘空间不足！"
...
remove() {
	...
	for package in "$@"; do
		echo -e "${riwi001}正在卸载 $package...${rw_bai}"
		if command -v dnf &>/dev/null; then
...
install_add_docker() {
	echo -e "${riwi001}正在安装docker环境...${rw_bai}"
...
while true; do
	...
	echo -e "${riwi001}Docker容器管理${rw_bai}"
	echo -e "${riwi001}------------------------${rw_bai}"
	echo ""
	echo -e "${rw_huang}功能说明:${rw_bai}"
	echo -e "${riwi001}  Docker容器的完整管理功能，包括:${rw_bai}"
	echo -e "${riwi001}  • 容器生命周期管理: 创建、启动、停止、删除、重启${rw_bai}"
	echo -e "${riwi001}  • 容器监控: 查看日志、资源占用、网络信息${rw_bai}"
	echo -e "${riwi001}  • 容器操作: 进入容器、端口管理${rw_bai}"
	echo ""
	echo -e "${rw_huang}提示: ${rw_bai}选择对应的数字即可进行相应操作${rw_bai}"
	echo ""
	echo -e "${riwi001}当前容器列表:${rw_bai}"
	docker ps -a --format "table {{.ID}}\t{{.Names}}\t{{.Status}}\t{{.Ports}}"
	echo ""
	echo -e "${riwi001}------------------------${rw_bai}"
	echo -e "${riwi001}1.   ${rw_bai}创建新容器 ${rw_huang}★${rw_bai}"
	echo -e "${riwi001}------------------------${rw_bai}"
	echo -e "${riwi001}2.   ${rw_bai}启动指定容器                   6.   ${rw_bai}启动所有容器${rw_bai}"
	echo -e "${riwi001}3.   ${rw_bai}停止指定容器                   7.   ${rw_bai}停止所有容器${rw_bai}"
	echo -e "${riwi001}4.   ${rw_bai}删除指定容器                   8.   ${rw_bai}删除所有容器${rw_bai}"
	echo -e "${riwi001}5.   ${rw_bai}重启指定容器                   9.   ${rw_bai}重启所有容器${rw_bai}"
	echo -e "${riwi001}------------------------${rw_bai}"
	echo -e "${riwi001}11.  ${rw_bai}进入指定容器                   12.  ${rw_bai}查看容器日志${rw_bai}"
	echo -e "${riwi001}13.  ${rw_bai}查看容器网络                   14.  ${rw_bai}查看容器资源占用${rw_bai}"
	echo -e "${riwi001}------------------------${rw_bai}"
	echo -e "${riwi001}15.  ${rw_bai}开启容器端口访问               16.  ${rw_bai}关闭容器端口访问${rw_bai}"
	echo -e "${riwi001}------------------------${rw_bai}"
	echo -e "${riwi001}0.   ${rw_bai}返回上一级选单${rw_bai}"
	echo -e "${riwi001}------------------------${rw_bai}"
	...
	case $sub_choice in
		...
		8)
			send_stats "删除所有容器"
			read -e -p "$(echo -e "${rw_hong}注意: ${rw_bai}确定删除所有容器吗？(Y/N): ")" choice
			case "$choice" in
			  [Yy])
...
while true; do
	...
	echo -e "${riwi001}Docker镜像管理${rw_bai}"
	echo -e "${riwi001}------------------------${rw_bai}"
	echo ""
	echo -e "${rw_huang}功能说明:${rw_bai}"
	echo -e "${riwi001}  Docker镜像的管理功能，包括:${rw_bai}"
	echo -e "${riwi001}  • 拉取/下载镜像: 从Docker Hub获取镜像${rw_bai}"
	echo -e "${riwi001}  • 更新镜像: 重新拉取最新版本的镜像${rw_bai}"
	echo -e "${riwi001}  • 删除镜像: 清理不需要的镜像以释放空间${rw_bai}"
	echo ""
	echo -e "${rw_huang}提示: ${rw_bai}选择对应的数字即可进行相应操作${rw_bai}"
	echo ""
	echo -e "${riwi001}当前镜像列表:${rw_bai}"
	docker image ls
	echo ""
	echo -e "${riwi001}------------------------${rw_bai}"
	echo -e "${riwi001}1.   ${rw_bai}获取指定镜像                   3.   ${rw_bai}删除指定镜像${rw_bai}"
	echo -e "${riwi001}2.   ${rw_bai}更新指定镜像                   4.   ${rw_bai}删除所有镜像${rw_bai}"
	echo -e "${riwi001}------------------------${rw_bai}"
	echo -e "${riwi001}0.   ${rw_bai}返回上一级选单${rw_bai}"
	echo -e "${riwi001}------------------------${rw_bai}"
	read -e -p "请输入你的选择: " sub_choice
	case $sub_choice in
		1)
			...
			for name in $imagenames; do
				echo -e "${riwi001}正在获取镜像: $name${rw_bai}"
				docker pull $name
			done
			;;
		2)
			...
			read -e -p "请输入镜像名（多个镜像名请用空格分隔）: " imagenames
			for name in $imagenames; do
				echo -e "${riwi001}正在更新镜像: $name${rw_bai}"
				docker pull $name
			done
			;;
		3)
		...
		4)
			send_stats "删除所有镜像"
			read -e -p "$(echo -e "${rw_hong}注意: ${rw_bai}确定删除所有镜像吗？(Y/N): ")" choice
			case "$choice" in
			  [Yy])
...
install_crontab() {
	...
	echo -e "${rw_lv}crontab 已安装且 cron 服务正在运行。${rw_bai}"
}
...
docker_ipv6_on() {
	...
	if [ ! -f "$CONFIG_FILE" ]; then
		...
		if [[ "$ORIGINAL_CONFIG" == "$UPDATED_CONFIG" ]]; then
			echo -e "${rw_huang}当前已开启ipv6访问${rw_bai}"
		else
...
docker_ipv6_off() {
	...
	# 对比原始配置与新配置
	if [[ "$CURRENT_IPV6" == "false" ]]; then
		echo -e "${rw_huang}当前已关闭ipv6访问${rw_bai}"
	else
		echo "$UPDATED_CONFIG" | jq . > "$CONFIG_FILE"
		restart docker
		echo -e "${rw_huang}已成功关闭ipv6访问${rw_bai}"
	fi
}
...
add_swap() {
	...
	echo -e "虚拟内存大小已调整为${rw_huang}${new_swap}${rw_bai}M"
}
...
install_ssltls_text() {
	echo -e "${rw_huang}$yuming 公钥信息${rw_bai}"
	cat /etc/letsencrypt/live/$yuming/fullchain.pem
	echo ""
	echo -e "${rw_huang}$yuming 私钥信息${rw_bai}"
	cat /etc/letsencrypt/live/$yuming/privkey.pem
	echo ""
	echo -e "${rw_huang}证书存放路径${rw_bai}"
...
add_ssl() {
echo -e "${rw_huang}快速申请SSL证书，过期前自动续签${rw_bai}"
yuming="${1:-}"
if [ -z "$yuming" ]; then
	add_yuming
fi
...
ssl_ps() {
	echo -e "${rw_huang}已申请的证书到期情况${rw_bai}"
	echo "站点信息                      证书到期时间"
	echo "------------------------"
...
certs_status() {
	...
	if [ -f "$file_path" ]; then
		...
		send_stats "域名证书申请失败"
		echo -e "${rw_hong}注意: ${rw_bai}证书申请失败，请检查以下可能原因并重试："
...
add_yuming() {
	  ip_address
	  echo -e "先将域名解析到本机IP: ${rw_huang}$ipv4_address  $ipv6_address${rw_bai}"
	  ...
	  while true; do
		...
		check_cf_mode
			  ...
			  case $sub_choice in
				  ...
				  22)
					  send_stats "高负载开启5秒盾"
					  echo -e "${rw_huang}网站每5分钟自动检测，当达检测到高负载会自动开盾，低负载也会自动关闭5秒盾。${rw_bai}"
					  echo "--------------"
					  echo "获取CF参数: "
					  echo -e "到cf后台右上角我的个人资料，选择左侧API令牌，获取${rw_huang}Global API Key${rw_bai}"
					  echo -e "到cf后台域名概要页面右下方获取${rw_huang}区域ID${rw_bai}"
					  echo "https://dash.cloudflare.com/login"
					  echo "--------------"
					  read -e -p "输入CF的账号: " cfuser
					  read -e -p "输入CF的Global API Key: " cftoken
...
check_docker_app() {
	if docker ps -a --format '{{.Names}}' 2>/dev/null | grep -q "$docker_name" ; then
		check_docker="${rw_lv}已安装${rw_bai}"
	else
		check_docker="${rw_hui}未安装${rw_bai}"
	fi
}
		...
		1)
			...
			while true; do
				...
				if ss -tuln | grep -q ":$app_port "; then
					echo -e "${rw_hong}错误: ${rw_bai}端口 $app_port 已被占用，请更换一个端口"
					send_stats "应用端口已被占用"
				else
...
check_f2b_status() {
	if command -v fail2ban-client >/dev/null 2>&1; then
		check_f2b_status="${rw_lv}已安装${rw_bai}"
	else
		check_f2b_status="${rw_hui}未安装${rw_bai}"
	fi
}
...
f2b_basic_config() {
	...
	if ! command -v fail2ban-client >/dev/null 2>&1; then
		echo -e "${rw_hui}未检测到 fail2ban-client，请先安装 fail2ban。${rw_bai}"
		return
	fi
...
EOF
	...
	echo -e "${rw_lv}已写入配置${rw_bai}: /etc/fail2ban/jail.d/sshd.local"
...
f2b_edit_config() {
	...
	if [ ! -d /etc/fail2ban ]; then
		echo -e "${rw_hui}/etc/fail2ban 不存在，请先安装 fail2ban。${rw_bai}"
		return
	fi
	...
	nano "$cfg"
	echo -e "${rw_lv}已保存${rw_bai}，正在 reload fail2ban..."
	fail2ban-client reload >/dev/null 2>&1 || true
}
...
server_reboot() {
	...
	read -e -p "$(echo -e "${rw_huang}提示: ${rw_bai}现在重启服务器吗？(Y/N): ")" rboot
	case "$rboot" in
	  [Yy])
		echo "已重启"
		reboot
...
ldnmp_install_status_one() {
   ...
   if docker inspect "php" &>/dev/null; then
	...
	echo -e "${rw_huang}提示: ${rw_bai}建站环境已安装。无需再次安装！"
	break_end
	linux_ldnmp
   fi
...
root_use
clear
echo -e "${rw_huang}LDNMP环境未安装，开始安装LDNMP环境...${rw_bai}"
...
root_use
clear
echo -e "${rw_huang}nginx未安装，开始安装nginx环境...${rw_bai}"
install_dependency
install_docker
install_certbot
install_ldnmp_conf
...
ldnmp_web_status() {
	...
	while true; do
		...
		echo "站点目录"
		echo "------------------------"
		echo -e "数据 ${rw_hui}/home/web/html${rw_bai}     证书 ${rw_hui}/home/web/certs${rw_bai}     配置 ${rw_hui}/home/web/conf.d${rw_bai}"
		echo "------------------------"
		echo ""
		echo "操作"
		echo "------------------------"
...
if [ -d "/home/frp/" ]; then
	check_frp="${rw_lv}已安装${rw_bai}"
else
	check_frp="${rw_hui}未安装${rw_bai}"
fi
...
yt_menu_pro() {
	...
	while true; do
		...
		if [ -x "/usr/local/bin/yt-dlp" ]; then
		   local YTDLP_STATUS="${rw_lv}已安装${rw_bai}"
		else
		   local YTDLP_STATUS="${rw_hui}未安装${rw_bai}"
		fi
...
linux_update() {
	clear
	echo -e "${riwi001}系统更新${rw_bai}"
	echo -e "${riwi001}------------------------${rw_bai}"
	echo ""
	echo -e "${rw_huang}功能说明:${rw_bai}"
	echo -e "${riwi001}  自动检测系统包管理器并更新所有软件包，包括:${rw_bai}"
	echo -e "${riwi001}  • Debian/Ubuntu: apt update && apt full-upgrade${rw_bai}"
	echo -e "${riwi001}  • CentOS/RHEL: yum update 或 dnf update${rw_bai}"
	echo -e "${riwi001}  • Alpine: apk update && apk upgrade${rw_bai}"
	echo -e "${riwi001}  • Arch: pacman -Syu${rw_bai}"
	echo ""
	echo -e "${rw_huang}提示: 更新过程可能需要几分钟，请耐心等待${rw_bai}"
	echo ""
	send_stats "系统更新"
	echo -e "${riwi001}正在系统更新...${rw_bai}"
	if command -v dnf &>/dev/null; then
		dnf -y update
	elif command -v yum &>/dev/null; then
		yum -y update
...
linux_clean() {
	clear
	echo -e "${riwi001}系统清理${rw_bai}"
	echo -e "${riwi001}------------------------${rw_bai}"
	echo ""
	echo -e "${rw_huang}功能说明:${rw_bai}"
	echo -e "${riwi001}  清理系统垃圾文件和缓存，释放磁盘空间，包括:${rw_bai}"
	echo -e "${riwi001}  • 清理包管理器缓存${rw_bai}"
	echo -e "${riwi001}  • 卸载不需要的依赖包${rw_bai}"
	echo -e "${riwi001}  • 清理旧的系统日志${rw_bai}"
	echo -e "${riwi001}  • 重建软件包数据库${rw_bai}"
	echo ""
	echo -e "${rw_huang}提示: 清理过程中可能需要几分钟，请耐心等待${rw_bai}"
	echo ""
	send_stats "系统清理"
	echo -e "${riwi001}正在系统清理...${rw_bai}"
	if command -v dnf &>/dev/null; then
		rpm --rebuilddb
		dnf autoremove -y
		dnf clean all
...
add_sshkey() {
	...
	ip_address
	echo -e "私钥信息已生成，务必复制保存，可保存成 ${rw_huang}${ipv4_address}_ssh.key${rw_bai} 文件，用于以后的SSH登录"
...
import_sshkey() {
	...
	if [[ -z "$public_key" ]]; then
		echo -e "${rw_hong}错误：未输入公钥内容。${rw_bai}"
		return 1
	fi

	if [[ ! "$public_key" =~ ^ssh-(rsa|ed25519|ecdsa) ]]; then
		echo -e "${rw_hong}错误：看起来不像合法的 SSH 公钥。${rw_bai}"
		return 1
	fi
...
sshkey_panel() {
  ...
  while true; do
	  ...
	  if [[ "$REAL_STATUS" =~ "yes" ]]; then
		  IS_KEY_ENABLED="${rw_lv}已启用${rw_bai}"
	  else
	  	  IS_KEY_ENABLED="${rw_hui}未启用${rw_bai}"
	  fi
...
add_sshpasswd() {
	...
	echo -e "${rw_lv}密码设置完毕，已更改为密码登录模式！${rw_bai}"
}
...
root_use() {
clear
[ "$EUID" -ne 0 ] && echo -e "${rw_huang}提示: ${rw_bai}该功能需要root用户才能运行！" && break_end && riwi
}
...
dd_xitong() {
		...
		dd_xitong_1() {
		  echo -e "重装后初始用户名: ${rw_huang}root${rw_bai}  初始密码: ${rw_huang}LeitboGi0ro${rw_bai}  初始端口: ${rw_huang}22${rw_bai}"
		  echo -e "${rw_huang}重装后请及时修改初始密码，防止暴力入侵。命令行输入passwd修改密码${rw_bai}"
		  echo -e "按任意键继续..."
		  read -n 1 -s -r -p ""
		  install wget
		  dd_xitong_MollyLau
		...
		dd_xitong_2() {
		  echo -e "重装后初始用户名: ${rw_huang}Administrator${rw_bai}  初始密码: ${rw_huang}Teddysun.com${rw_bai}  初始端口: ${rw_huang}3389${rw_bai}"
		  echo -e "按任意键继续..."
		  read -n 1 -s -r -p ""
		  install wget
		  dd_xitong_MollyLau
		...
		dd_xitong_3() {
		  echo -e "重装后初始用户名: ${rw_huang}root${rw_bai}  初始密码: ${rw_huang}123@@@${rw_bai}  初始端口: ${rw_huang}22${rw_bai}"
		  echo -e "按任意键继续..."
		  read -n 1 -s -r -p ""
		  dd_xitong_bin456789
		}
		...
		dd_xitong_4() {
		  echo -e "重装后初始用户名: ${rw_huang}Administrator${rw_bai}  初始密码: ${rw_huang}123@@@${rw_bai}  初始端口: ${rw_huang}3389${rw_bai}"
		  echo -e "按任意键继续..."
		  read -n 1 -s -r -p ""
		  dd_xitong_bin456789
		}
		  ...
		  while true; do
			...
			echo -e "${rw_hui}感谢bin456789大佬和leitbogioro大佬的脚本支持！${rw_bai} "
			echo -e "${rw_hui}bin456789项目地址: ${gh_https_url}github.com/bin456789/reinstall${rw_bai}"
			echo -e "${rw_hui}leitbogioro项目地址: ${gh_https_url}github.com/leitbogioro/Tools${rw_bai}"
			echo "------------------------"
...
clamav_scan() {
	...
	echo -e "${rw_lv}$@ 扫描完成，病毒报告存放在${rw_huang}/home/docker/clamav/log/scan.log${rw_bai}"
	echo -e "${rw_lv}如果有病毒请在${rw_huang}scan.log${rw_lv}文件中搜索FOUND关键字确认病毒位置 ${rw_bai}"
...
clamav() {
		  ...
		  while true; do
				...
				echo "------------------------"
				echo -e "${rw_lv}1. 全盘扫描 ${rw_bai}             ${rw_huang}2. 重要目录扫描 ${rw_bai}            ${riwi001} 3. 自定义目录扫描 ${rw_bai}"
				echo "------------------------"
				echo "0. 返回上一级选单"
				echo "------------------------"
				read -e -p "请输入你的选择: " sub_choice
...
_kernel_optimize_core() {
	...
	echo -e "${rw_lv}切换到${mode_name}...${rw_bai}"
...
SYSCTL
	...
	echo -e "${rw_lv}已应用 ${applied} 项参数${skipped:+，跳过 ${skipped} 项不支持的参数}${rw_bai}"
...
LIMITS
	...
	echo -e "${rw_lv}${mode_name} 优化完成！配置已持久化到 ${CONF}${rw_bai}"
	echo -e "${rw_lv}内存: ${MEM_MB}MB | 拥塞算法: ${CC} | 队列: ${QDISC}${rw_bai}"
}
...
restore_defaults() {
	...
	echo -e "${rw_lv}系统已还原到默认设置${rw_bai}"
}
...
Kernel_optimize() {
	...
	while true; do
	  ...
	  if [ -n "$current_mode" ]; then
		  echo -e "当前模式: ${rw_lv}${current_mode}${rw_bai}"
	  else
		  echo -e "当前模式: ${rw_hui}未优化${rw_bai}"
	  fi
	  echo "视频介绍: https://www.bilibili.com/video/BV1Kb421J7yg?t=0.1"
	  echo "------------------------------------------------"
	  echo "提供多种系统参数调优模式，用户可以根据自身使用场景进行选择切换。"
	  echo -e "${rw_huang}提示: ${rw_bai}生产环境请谨慎使用！"
	  echo -e "--------------------"
	  ...
	  echo -e "7. 自动调优：           根据测试数据自动调优内核参数。${rw_huang}★${rw_bai}"
	  echo "--------------------"
	  echo "0. 返回上一级选单"
	  echo "--------------------"
	  read -e -p "请输入你的选择: " sub_choice
	...
	if [ -f /etc/os-release ]; then
		...
		case $ID in
			debian|ubuntu|kali)
				...
				export LANG=${lang}
				echo -e "${rw_lv}系统语言已经修改为: $lang 重新连接SSH生效。${rw_bai}"
				hash -r
				break_end
...
echo -e "${rw_lv}变更完成。重新连接SSH后可查看变化！${rw_bai}"
...
shell_bianse() {
  ...
  while true; do
	...
	echo "命令行美化工具"
	echo "------------------------"
	echo -e "1. \033[1;32mroot \033[1;34mlocalhost \033[1;31m~ \033[0m${rw_bai}#"
	echo -e "2. \033[1;35mroot \033[1;36mlocalhost \033[1;33m~ \033[0m${rw_bai}#"
	echo -e "3. \033[1;31mroot \033[1;32mlocalhost \033[1;34m~ \033[0m${rw_bai}#"
	echo -e "4. \033[1;36mroot \033[1;33mlocalhost \033[1;37m~ \033[0m${rw_bai}#"
	echo -e "5. \033[1;37mroot \033[1;31mlocalhost \033[1;32m~ \033[0m${rw_bai}#"
	echo -e "6. \033[1;33mroot \033[1;34mlocalhost \033[1;35m~ \033[0m${rw_bai}#"
...
linux_trash() {
  ...
  while true; do
	...
	if ! grep -q "trash-put" "$bashrc_profile"; then
		trash_status="${rw_hui}未启用${rw_bai}"
	else
		trash_status="${rw_lv}已启用${rw_bai}"
	fi
...
disk_manager() {
	...
	while true; do
		clear
		echo "硬盘分区管理"
		echo -e "${rw_huang}该功能内部测试阶段，请勿在生产环境使用。${rw_bai}"
		echo "------------------------"
		list_partitions
		echo "------------------------"
...
linux_info() {
	...
	clear
	echo -e "${riwi001}系统查询${rw_bai}"
	echo -e "${riwi001}------------------------${rw_bai}"
	echo ""
	echo -e "${rw_huang}功能说明:${rw_bai}"
	echo -e "${riwi001}  查询系统的基本信息，包括:${rw_bai}"
	echo -e "${riwi001}  • IP地址和网络信息${rw_bai}"
	echo -e "${riwi001}  • CPU型号和使用率${rw_bai}"
	echo -e "${riwi001}  • 内存使用情况${rw_bai}"
	echo -e "${riwi001}  • 磁盘使用情况${rw_bai}"
	echo ""
	echo -e "${rw_huang}提示: 信息将自动显示，查看后按回车继续${rw_bai}"
	echo ""
	send_stats "系统信息查询"
...
linux_tools() {
  ...
  while true; do
	  ...
	  echo ""
	  echo -e "${rw_huang}功能说明:${rw_bai}"
	  echo -e "${riwi001}  提供常用系统工具的安装和管理，包括:${rw_bai}"
	  echo -e "${riwi001}  • 网络工具: curl, wget, socat${rw_bai}"
	  echo -e "${riwi001}  • 系统监控: htop, iftop, btop${rw_bai}"
	  echo -e "${riwi001}  • 文件工具: unzip, tar, ranger, ncdu${rw_bai}"
	  echo -e "${riwi001}  • 编辑器: vim, nano${rw_bai}"
	  echo -e "${riwi001}  • 开发工具: git, brew, nodejs, pnpm${rw_bai}"
	  echo ""
	  ...
	  echo -e "${riwi001}------------------------"
	  echo -e "${riwi001}1.   ${rw_bai}curl 下载工具 ${rw_huang}★${rw_bai}                   ${riwi001}2.   ${rw_bai}wget 下载工具 ${rw_huang}★${rw_bai}"
	  echo -e "${riwi001}3.   ${rw_bai}sudo 超级管理权限工具             ${riwi001}4.   ${rw_bai}socat 通信连接工具"
	  echo -e "${riwi001}5.   ${rw_bai}htop 系统监控工具                 ${riwi001}6.   ${rw_bai}iftop 网络流量监控工具"
	  echo -e "${riwi001}7.   ${rw_bai}unzip ZIP压缩解压工具             ${riwi001}8.   ${rw_bai}tar GZ压缩解压工具"
	  echo -e "${riwi001}9.   ${rw_bai}tmux 多路后台运行工具             ${riwi001}10.  ${rw_bai}ffmpeg 视频编码直播推流工具"
	  echo -e "${riwi001}------------------------"
	  echo -e "${riwi001}11.  ${rw_bai}btop 现代化监控工具 ${rw_huang}★${rw_bai}             ${riwi001}12.  ${rw_bai}ranger 文件管理工具"
	  echo -e "${riwi001}13.  ${rw_bai}ncdu 磁盘占用查看工具             ${riwi001}14.  ${rw_bai}fzf 全局搜索工具"
	  echo -e "${riwi001}15.  ${rw_bai}vim 文本编辑器                    ${riwi001}16.  ${rw_bai}nano 文本编辑器 ${rw_huang}★${rw_bai}"
	  echo -e "${riwi001}17.  ${rw_bai}git 版本控制系统                  ${riwi001}18.  ${rw_bai}opencode AI编程助手 ${rw_huang}★${rw_bai}"
	  echo -e "${riwi001}------------------------"
	  echo -e "${riwi001}19.  ${rw_bai}brew macOS包管理器                ${riwi001}20.  ${rw_bai}nodejs JavaScript运行时 ${rw_huang}★${rw_bai}"
	  echo -e "${riwi001}21.  ${rw_bai}pnpm 快速高效的包管理器 ${rw_huang}★${rw_bai}"
	  echo -e "${riwi001}------------------------"
	  echo -e "${riwi001}31.  ${rw_bai}全部安装${rw_bai}"
	  echo -e "${riwi001}33.  ${rw_bai}全部卸载"
	  echo -e "${riwi001}------------------------"
	  echo -e "${riwi001}41.  ${rw_bai}安装指定工具                      ${riwi001}42.  ${rw_bai}卸载指定工具"
	  echo -e "${riwi001}------------------------"
	  ...
	  case $sub_choice in
		  ...
		  2)
			...
			19)
			  ...
			  if [ $? -eq 0 ]; then
				echo -e "${rw_lv}brew 安装成功${rw_bai}"
				echo -e "${rw_huang}提示: 请运行以下命令添加到环境变量:${rw_bai}"
			...
			20)
			  ...
			  if command -v node >/dev/null 2>&1; then
				echo -e "${rw_lv}Node.js 安装成功${rw_bai}"
				node --version
				npm --version
			  else
...
docker_ssh_migration() {
	...
	backup_docker() {
		...
		echo -e "${riwi001}正在备份 Docker 容器...${rw_bai}"
		...
		[[ ${#TARGET_CONTAINERS[@]} -eq 0 ]] && { echo -e "${rw_hong}没有找到容器${rw_bai}"; return; }
		...
		for c in "${TARGET_CONTAINERS[@]}"; do
			...
			if is_compose_container "$c"; then
				...
				if [[ -n "${PACKED_COMPOSE_PATHS[$project_dir]}" ]]; then
					echo -e "${rw_huang}Compose 项目 [$project_name] 已备份过，跳过重复打包...${rw_bai}"
					continue
				fi
				...
				if [ -f "$project_dir/docker-compose.yml" ]; then
					...
					echo -e "${rw_lv}Compose 项目 [$project_name] 已打包: ${project_dir}${rw_bai}"
				else
					echo -e "${rw_hong}未找到 docker-compose.yml，跳过此容器...${rw_bai}"
				fi
			else
				# 普通容器备份卷
				local VOL_PATHS
		...
		if [ -d "/home/docker" ]; then
			echo -e "${riwi001}备份 /home/docker 下的文件...${rw_bai}"
			find /home/docker -maxdepth 1 -type f | tar -czf "${BACKUP_DIR}/home_docker_files.tar.gz" -T -
			echo -e "${rw_lv}/home/docker 下的文件已打包到: ${BACKUP_DIR}/home_docker_files.tar.gz${rw_bai}"
		fi

		chmod +x "$RESTORE_SCRIPT"
		echo -e "${rw_lv}备份完成: ${BACKUP_DIR}${rw_bai}"
		echo -e "${rw_lv}可用还原脚本: ${RESTORE_SCRIPT}${rw_bai}"
	...
	restore_docker() {
		...
		[[ ! -d "$BACKUP_DIR" ]] && { echo -e "${rw_hong}备份目录不存在${rw_bai}"; return; }

		echo -e "${riwi001}开始执行还原操作...${rw_bai}"
		...
		for f in "$BACKUP_DIR"/backup_type_*; do
			...
			if grep -q "compose" "$f"; then
				...
				if [[ "$running_count" -gt 0 ]]; then
					echo -e "${rw_huang}Compose 项目 [$project_name] 已有容器在运行，跳过还原...${rw_bai}"
					continue
				fi
				...
				echo -e "${rw_lv}Compose 项目 [$project_name] 已解压到: $original_path${rw_bai}"

				cd "$original_path" || return
				docker compose down || true
				docker compose up -d
				echo -e "${rw_lv}Compose 项目 [$project_name] 还原完成！${rw_bai}"
			fi
		done

		# --------- 继续还原普通容器 ---------
		echo -e "${riwi001}检查并还原普通 Docker 容器...${rw_bai}"
		local has_container=false
		for json in "$BACKUP_DIR"/*_inspect.json; do
			[[ ! -f "$json" ]] && continue
			has_container=true
			container=$(basename "$json" | sed 's/_inspect.json//')
			echo -e "${rw_lv}处理容器: $container${rw_bai}"

			# 检查容器是否已经存在且正在运行
			if docker ps --format '{{.Names}}' | grep -q "^${container}$"; then
				echo -e "${rw_huang}容器 [$container] 已在运行，跳过还原...${rw_bai}"
				continue
			fi

			IMAGE=$(jq -r '.[0].Config.Image' "$json")
			[[ -z "$IMAGE" || "$IMAGE" == "null" ]] && { echo -e "${rw_hong}未找到镜像信息，跳过: $container${rw_bai}"; continue; }
			...
			if docker ps -a --format '{{.Names}}' | grep -q "^${container}$"; then
				echo -e "${rw_huang}容器 [$container] 存在但未运行，删除旧容器...${rw_bai}"
		...
		[[ "$has_container" == false ]] && echo -e "${rw_huang}未找到普通容器的备份信息${rw_bai}"

		# 还原 /home/docker 下的文件
		if [ -f "$BACKUP_DIR/home_docker_files.tar.gz" ]; then
			echo -e "${riwi001}正在还原 /home/docker 下的文件...${rw_bai}"
			mkdir -p /home/docker
			tar -xzf "$BACKUP_DIR/home_docker_files.tar.gz" -C /
			echo -e "${rw_lv}/home/docker 下的文件已还原完成${rw_bai}"
		else
			echo -e "${rw_huang}未找到 /home/docker 下文件的备份，跳过...${rw_bai}"
		fi
	...
	delete_backup() {
		...
		[[ ! -d "$BACKUP_DIR" ]] && { echo -e "${rw_hong}备份目录不存在${rw_bai}"; return; }
		rm -rf "$BACKUP_DIR"
		echo -e "${rw_lv}已删除备份: ${BACKUP_DIR}${rw_bai}"
	}
...
linux_docker() {
	...
	while true; do
	  clear
	  send_stats "Docker管理"
	  echo -e "${riwi001}Docker管理${rw_bai}"
	  echo -e "${riwi001}------------------------${rw_bai}"
	  echo ""
	  echo -e "${rw_huang}功能说明:${rw_bai}"
	  echo -e "${riwi001}  Docker容器化环境的完整管理功能，包括:${rw_bai}"
	  echo -e "${riwi001}  • 安装/更新Docker环境${rw_bai}"
	  echo -e "${riwi001}  • 查看Docker全局状态${rw_bai}"
	  echo -e "${riwi001}  • 管理容器、镜像、网络、卷${rw_bai}"
	  echo -e "${riwi001}  • 清理无用的Docker资源${rw_bai}"
	  echo ""
	  echo -e "${rw_huang}提示: 选择对应的数字即可进行相应操作${rw_bai}"
	  echo ""
	  docker_tato
	  echo -e "${riwi001}------------------------"
	  echo -e "${riwi001}1.   ${rw_bai}安装更新Docker环境 ${rw_huang}★${rw_bai}"
	  echo -e "${riwi001}------------------------"
	  echo -e "${riwi001}2.   ${rw_bai}查看Docker全局状态 ${rw_huang}★${rw_bai}"
	  echo -e "${riwi001}------------------------"
	  echo -e "${riwi001}3.   ${rw_bai}Docker容器管理 ${rw_huang}★${rw_bai}"
	  echo -e "${riwi001}4.   ${rw_bai}Docker镜像管理"
	  echo -e "${riwi001}5.   ${rw_bai}Docker网络管理"
	  echo -e "${riwi001}6.   ${rw_bai}Docker卷管理"
	  echo -e "${riwi001}------------------------"
	  echo -e "${riwi001}7.   ${rw_bai}清理无用的docker容器和镜像网络数据卷"
	  echo -e "${riwi001}------------------------"
...
linux_Oracle() {
	 ...
	 while true; do
	  ...
	  echo -e "甲骨文云脚本合集"
	  echo -e "${riwi001}------------------------"
	  echo -e "${riwi001}1.   ${rw_bai}安装闲置机器活跃脚本"
	  echo -e "${riwi001}2.   ${rw_bai}卸载闲置机器活跃脚本"
	  echo -e "${riwi001}------------------------"
	  echo -e "${riwi001}3.   ${rw_bai}DD重装系统脚本"
	  echo -e "${riwi001}4.   ${rw_bai}R探长开机脚本"
	  echo -e "${riwi001}5.   ${rw_bai}开启ROOT密码登录模式"
	  echo -e "${riwi001}6.   ${rw_bai}IPV6恢复工具"
	  echo -e "${riwi001}------------------------"
	  echo -e "${riwi001}0.   ${rw_bai}返回主菜单"
	  echo -e "${riwi001}------------------------${rw_bai}"
	  read -e -p "请输入你的选择: " sub_choice
...
docker_tato() {
	...
	if command -v docker &> /dev/null; then
		echo -e "${riwi001}------------------------"
		echo -e "${rw_lv}环境已经安装${rw_bai}  容器: ${rw_lv}$container_count${rw_bai}  镜像: ${rw_lv}$image_count${rw_bai}  网络: ${rw_lv}$network_count${rw_bai}  卷: ${rw_lv}$volume_count${rw_bai}"
	fi
}
...
if command -v docker &>/dev/null; then
	if docker ps --filter "name=nginx" --filter "status=running" | grep -q nginx; then
		echo -e "${rw_huang}------------------------"
		echo -e "${rw_lv}环境已安装${rw_bai}  站点: $output  数据库: $db_output"
	fi
fi
...
moltbot_menu() {
	...
	check_openclaw_update() {
		...
		if [ "$local_version" != "$remote_version" ]; then
			echo "${rw_huang}检测到新版本:$remote_version${rw_bai}"
		else
			echo "${rw_lv}当前版本已是最新:$local_version${rw_bai}"
		fi
	}
	...
	get_install_status() {
		if command -v openclaw >/dev/null 2>&1; then
			echo "${rw_lv}已安装${rw_bai}"
		else
			echo "${rw_hui}未安装${rw_bai}"
		fi
	}

	get_running_status() {		
		if pgrep -f "openclaw.*gateway" >/dev/null 2>&1; then
			echo "${rw_lv}运行中${rw_bai}"
		else
			echo "${rw_hui}未运行${rw_bai}"
		fi
	}
	...
	while IFS=$'\t' read -r rec_type idx name base_url model_count api_type latency_txt latency_level; do
		case "$rec_type" in
			...
			ROW)
				...
				printf '%b\n' "[$idx] ${name} | API: ${base_url} | 协议: ${api_type} | 模型数量: ${rw_huang}${model_count}${rw_bai} | 延迟/状态: ${latency_color}${latency_txt}${rw_bai}"
				;;
		esac
...
PY
	...
	openclaw_api_providers_showcase() {
		...
		echo ""
		echo -e "${riwi001}╔════════════════════════════════════════════════════════════╗${rw_bai}"
		echo -e "${riwi001}║${rw_bai}            ${rw_huang}🌟 API 厂商推荐列表${rw_bai}                          ${riwi001}║${rw_bai}"
		echo -e "${riwi001}║${rw_bai}            ${rw_zi}部分入口含 AFF${rw_bai}                            ${riwi001}║${rw_bai}"
		echo -e "${riwi001}╚════════════════════════════════════════════════════════════╝${rw_bai}"
		echo ""
		echo -e "  ${rw_lv}● DeepSeek${rw_bai}"
		echo -e "    ${riwi001}https://api-docs.deepseek.com/${rw_bai}"
		echo ""
		echo -e "  ${rw_lv}● OpenRouter${rw_bai}"
		echo -e "    ${riwi001}https://openrouter.ai/${rw_bai}"
		echo ""
		echo -e "  ${rw_lv}● Kimi${rw_bai}"
		echo -e "    ${riwi001}https://platform.moonshot.cn/docs/guide/start-using-kimi-api${rw_bai}"
		echo ""
		echo -e "  ${rw_lv}● 超算互联网${rw_bai}"
		echo -e "    ${riwi001}https://www.scnet.cn/${rw_bai}"
		echo ""
		echo -e "  ${rw_huang}● 优云智算${rw_bai} ${rw_zi}[AFF]${rw_bai}"
		echo -e "    ${riwi001}https://passport.compshare.cn/register?referral_code=4mscFZXfutfFi8swMVsPuf${rw_bai}"
		echo ""
		echo -e "  ${rw_huang}● 硅基流动${rw_bai} ${rw_zi}[AFF]${rw_bai}"
		echo -e "    ${riwi001}https://cloud.siliconflow.cn/i/irWVdPic${rw_bai}"
		echo ""
		echo -e "  ${rw_huang}● 智谱 GLM${rw_bai} ${rw_zi}[AFF]${rw_bai}"
		echo -e "    ${riwi001}https://www.bigmodel.cn/glm-coding?ic=HYOTDOAJMR${rw_bai}"
		echo ""
		echo -e "  ${rw_huang}● PackyAPI${rw_bai} ${rw_zi}[AFF]${rw_bai}"
		echo -e "    ${riwi001}https://www.packyapi.com/register?aff=wHri${rw_bai}"
		echo ""
		echo -e "  ${rw_huang}● 云雾 API${rw_bai} ${rw_zi}[AFF]${rw_bai}"
		echo -e "    ${riwi001}https://yunwu.ai/register?aff=ZuyK${rw_bai}"
		echo ""
		echo -e "  ${rw_huang}● 柏拉图AI${rw_bai} ${rw_zi}[AFF]${rw_bai}"
		echo -e "    ${riwi001}https://api.bltcy.ai/register?aff=TBzb114019${rw_bai}"
		echo ""
		echo -e "  ${rw_lv}● MiniMax${rw_bai}"
		echo -e "    ${riwi001}https://www.minimaxi.com/${rw_bai}"
		echo ""
		echo -e "  ${rw_lv}● NVIDIA${rw_bai}"
		echo -e "    ${riwi001}https://build.nvidia.com/settings/api-keys${rw_bai}"
		echo ""
		echo -e "  ${rw_lv}● Ollama${rw_bai}"
		echo -e "    ${riwi001}https://ollama.com/${rw_bai}"
		echo ""
		echo -e "  ${rw_lv}● 白山云${rw_bai}"
		echo -e "    ${riwi001}https://ai.baishan.com/${rw_bai}"
		echo ""
		echo -e "${riwi001}────────────────────────────────────────────────────────────${rw_bai}"
		echo -e "  ${rw_zi}图例：${rw_lv}● 官方入口${rw_bai}  ${rw_huang}● AFF 推荐入口${rw_bai}"
		echo ""
		echo -e "${rw_huang}提示：复制链接到浏览器打开即可访问${rw_bai}"
		echo ""
...
openclaw_json_get_bool() {
	...
	openclaw_colorize_bot_status() {
		local status="$1"
		case "$status" in
			已连接) echo -e "${rw_lv}${status}${rw_bai}" ;;
			已配置) echo -e "${rw_huang}${status}${rw_bai}" ;;
			异常) echo -e "${rw_hong}${status}${rw_bai}" ;;
			*) echo "$status" ;;
		esac
	}
...
' "$approvals_file" "$sec" "$ask" "$fallback")
	...
	openclaw_permission_render_status() {
		...
		echo "---------------------------------------"
		echo -e "${rw_huang}[应用层 Tool Policy 状态]${rw_bai}"
		...
		echo -e "\n${rw_huang}[底层 Exec Approvals 状态]${rw_bai}"
		if openclaw_has_command openclaw; then
...
'
	...
	openclaw_permission_apply_standard() {
		...
		openclaw_permission_restart_gateway
		echo -e "${rw_lv}✅ 已切换为标准安全模式 (所有危险命令将通过UI/TG请求你的审批)${rw_bai}"
	}
	...
	openclaw_permission_apply_developer() {
		...
		openclaw_permission_restart_gateway
		echo -e "${rw_lv}✅ 已切换为开发增强模式 (允许提权，但常规危险命令依然需要审批)${rw_bai}"
	}
	...
	openclaw_permission_apply_full() {
		...
		openclaw_permission_restart_gateway
		echo -e "${rw_lv}✅ 已切换为完全开放模式 (警告：所有宿主机命令拦截已失效，智能体具有最高权限)${rw_bai}"
	}
	...
	openclaw_permission_restore_official_defaults() {
		...
		openclaw_permission_restart_gateway
		echo -e "${rw_lv}✅ 已恢复到 OpenClaw 官方安全沙盒防御机制${rw_bai}"
	}
...
' "$allowlist_json"
	...
	openclaw_permission_menu() {
		...
		while true; do
			...
			openclaw_permission_render_status
			echo "---------------------------------------"
			echo -e "${riwi001}1.${rw_bai} 切换为标准安全模式（日常推荐，弹卡片审批）"
			echo -e "${riwi001}2.${rw_bai} 切换为开发增强模式（允许智能体申请提权）"
			echo -e "${riwi001}3.${rw_bai} 切换为完全开放模式（${rw_hong}高风险！彻底解除所有宿主机拦截${rw_bai}）"
			echo -e "${riwi001}4.${rw_bai} 恢复官方默认沙盒防御策略"
			echo -e "${riwi001}5.${rw_bai} 运行底层安全审计与自动修复"
			echo -e "${riwi001}6.${rw_bai} 管理 Exec 命令白名单"
			echo -e "${riwi001}0.${rw_bai} 返回上一级"
			echo "---------------------------------------"
			...
			case "$perm_choice" in
				...
				2)
					...
					;;
				3)
					echo -e "${rw_hong}⚠️ 完全开放模式会彻底瓦解 exec 审批并自动放行高危代码。${rw_bai}"
...
openclaw_backup_restore_menu() {
	...
	openclaw_domain_webui() {
		...
		read
		echo -e "${riwi001}正在加载设备列表……${rw_bai}"
		...
		if [ -f "$config_file" ]; then
			...
			if command -v jq >/dev/null 2>&1; then
				...
				echo -e "${riwi001}已将域名 ${yuming} 加入 allowedOrigins 配置${rw_bai}"
				openclaw gateway restart >/dev/null 2>&1
			fi
		fi
...
cd ~
send_stats "应用市场"
echo -e "${riwi001}应用市场${rw_bai}"
echo -e "${riwi001}------------------------${rw_bai}"
echo ""
echo -e "${rw_huang}功能说明:${rw_bai}"
echo -e "${riwi001}  提供各种实用应用程序的一键安装，包括:${rw_bai}"
echo -e "${riwi001}  • 网站程序: WordPress, Discuz, 可道云等${rw_bai}"
echo -e "${riwi001}  • 开发工具: VS Code Server, Jupyter等${rw_bai}"
echo -e "${riwi001}  • 系统工具: 监控面板, 管理面板等${rw_bai}"
echo -e "${riwi001}  • 媒体工具: Jellyfin, 下载器等${rw_bai}"
echo ""
echo -e "${rw_huang}提示: 应用列表将自动更新，请稍等...${rw_bai}"
echo ""
install git
echo -e "${riwi001}正在更新应用列表请稍等……${rw_bai}"
if [ ! -d apps/.git ]; then
...
while true; do
	...
	if [ -z "$sub_choice" ]; then
	  ...
	  for i in {1..150}; do
		  if echo "$app_numbers" | grep -q "^$i$"; then
			  ...
			  declare "color$i=${rw_bai}"
		  fi
	  done

	  echo -e "${riwi001}1.   ${color1}宝塔面板官方版                      ${riwi001}2.   ${color2}aaPanel宝塔国际版"
	  echo -e "${riwi001}3.   ${color3}1Panel新一代管理面板                ${riwi001}4.   ${color4}NginxProxyManager可视化面板"
	  echo -e "${riwi001}5.   ${color5}OpenList多存储文件列表程序          ${riwi001}6.   ${color6}Ubuntu远程桌面网页版"
	  echo -e "${riwi001}7.   ${color7}哪吒探针VPS监控面板                 ${riwi001}8.   ${color8}QB离线BT磁力下载面板"
	  echo -e "${riwi001}9.   ${color9}Poste.io邮件服务器程序              ${riwi001}10.  ${color10}RocketChat多人在线聊天系统"
	  echo -e "${riwi001}-------------------------"
	  echo -e "${riwi001}11.  ${color11}禅道项目管理软件                    ${riwi001}12.  ${color12}青龙面板定时任务管理平台"
	  echo -e "${riwi001}13.  ${color13}Cloudreve网盘 ${rw_huang}★${rw_bai}                     ${riwi001}14.  ${color14}简单图床图片管理程序"
	  echo -e "${riwi001}15.  ${color15}emby多媒体管理系统                  ${riwi001}16.  ${color16}Speedtest测速面板"
	  echo -e "${riwi001}17.  ${color17}AdGuardHome去广告软件               ${riwi001}18.  ${color18}onlyoffice在线办公OFFICE"
	  echo -e "${riwi001}19.  ${color19}雷池WAF防火墙面板                   ${riwi001}20.  ${color20}portainer容器管理面板"
	  echo -e "${riwi001}-------------------------"
	  echo -e "${riwi001}21.  ${color21}VScode网页版                        ${riwi001}22.  ${color22}UptimeKuma监控工具"
	  echo -e "${riwi001}23.  ${color23}Memos网页备忘录                     ${riwi001}24.  ${color24}Webtop远程桌面网页版 ${rw_huang}★${rw_bai}"
	  echo -e "${riwi001}25.  ${color25}Nextcloud网盘                       ${riwi001}26.  ${color26}QD-Today定时任务管理框架"
	  echo -e "${riwi001}27.  ${color27}Dockge容器堆栈管理面板              ${riwi001}28.  ${color28}LibreSpeed测速工具"
	  echo -e "${riwi001}29.  ${color29}searxng聚合搜索站 ${rw_huang}★${rw_bai}                 ${riwi001}30.  ${color30}PhotoPrism私有相册系统"
	  echo -e "${riwi001}-------------------------"
	  echo -e "${riwi001}31.  ${color31}StirlingPDF工具大全                 ${riwi001}32.  ${color32}drawio免费的在线图表软件 ${rw_huang}★${rw_bai}"
	  echo -e "${riwi001}33.  ${color33}Sun-Panel导航面板                   ${riwi001}34.  ${color34}Pingvin-Share文件分享平台"
	  echo -e "${riwi001}35.  ${color35}极简朋友圈                          ${riwi001}36.  ${color36}LobeChatAI聊天聚合网站"
	  echo -e "${riwi001}37.  ${color37}MyIP工具箱 ${rw_huang}★${rw_bai}                        ${riwi001}38.  ${color38}小雅alist全家桶"
	  echo -e "${riwi001}39.  ${color39}Bililive直播录制工具                ${riwi001}40.  ${color40}webssh网页版SSH连接工具"
	  echo -e "${riwi001}-------------------------"
	  echo -e "${riwi001}41.  ${color41}耗子管理面板                	 ${riwi001}42.  ${color42}Nexterm远程连接工具"
	  echo -e "${riwi001}43.  ${color43}RustDesk远程桌面(服务端) ${rw_huang}★${rw_bai}          ${riwi001}44.  ${color44}RustDesk远程桌面(中继端) ${rw_huang}★${rw_bai}"
	  echo -e "${riwi001}45.  ${color45}Docker加速站            		 ${riwi001}46.  ${color46}GitHub加速站 ${rw_huang}★${rw_bai}"
	  echo -e "${riwi001}47.  ${color47}普罗米修斯监控			 ${riwi001}48.  ${color48}普罗米修斯(主机监控)"
	  echo -e "${riwi001}49.  ${color49}普罗米修斯(容器监控)		 ${riwi001}50.  ${color50}补货监控工具"
	  echo -e "${riwi001}-------------------------"
	  echo -e "${riwi001}51.  ${color51}PVE开小鸡面板			 ${riwi001}52.  ${color52}DPanel容器管理面板"
	  echo -e "${riwi001}53.  ${color53}llama3聊天AI大模型                  ${riwi001}54.  ${color54}AMH主机建站管理面板"
	  echo -e "${riwi001}55.  ${color55}FRP内网穿透(服务端) ${rw_huang}★${rw_bai}	         ${riwi001}56.  ${color56}FRP内网穿透(客户端) ${rw_huang}★${rw_bai}"
	  echo -e "${riwi001}57.  ${color57}Deepseek聊天AI大模型                ${riwi001}58.  ${color58}Dify大模型知识库 ${rw_huang}★${rw_bai}"
	  echo -e "${riwi001}59.  ${color59}NewAPI大模型资产管理                ${riwi001}60.  ${color60}JumpServer开源堡垒机"
	  echo -e "${riwi001}-------------------------"
	  echo -e "${riwi001}61.  ${color61}在线翻译服务器			 ${riwi001}62.  ${color62}RAGFlow大模型知识库"
	  echo -e "${riwi001}63.  ${color63}OpenWebUI自托管AI平台 ${rw_huang}★${rw_bai}             ${riwi001}64.  ${color64}it-tools工具箱"
	  echo -e "${riwi001}65.  ${color65}n8n自动化工作流平台 ${rw_huang}★${rw_bai}               ${riwi001}66.  ${color66}yt-dlp视频下载工具"
	  echo -e "${riwi001}67.  ${color67}ddns-go动态DNS管理工具 ${rw_huang}★${rw_bai}            ${riwi001}68.  ${color68}AllinSSL证书管理平台"
	  echo -e "${riwi001}69.  ${color69}SFTPGo文件传输工具                  ${riwi001}70.  ${color70}AstrBot聊天机器人框架"
	  echo -e "${riwi001}-------------------------"
	  echo -e "${riwi001}71.  ${color71}Navidrome私有音乐服务器             ${riwi001}72.  ${color72}bitwarden密码管理器 ${rw_huang}★${rw_bai}"
	  echo -e "${riwi001}73.  ${color73}LibreTV私有影视                     ${riwi001}74.  ${color74}MoonTV私有影视"
	  echo -e "${riwi001}75.  ${color75}Melody音乐精灵                      ${riwi001}76.  ${color76}在线DOS老游戏"
	  echo -e "${riwi001}77.  ${color77}迅雷离线下载工具                    ${riwi001}78.  ${color78}PandaWiki智能文档管理系统"
	  echo -e "${riwi001}79.  ${color79}Beszel服务器监控                    ${riwi001}80.  ${color80}linkwarden书签管理"
	  echo -e "${riwi001}-------------------------"
	  echo -e "${riwi001}81.  ${color81}JitsiMeet视频会议                   ${riwi001}82.  ${color82}gpt-load高性能AI透明代理"
	  echo -e "${riwi001}83.  ${color83}komari服务器监控工具                ${riwi001}84.  ${color84}Wallos个人财务管理工具"
	  echo -e "${riwi001}85.  ${color85}immich图片视频管理器                ${riwi001}86.  ${color86}jellyfin媒体管理系统"
	  echo -e "${riwi001}87.  ${color87}SyncTV一起看片神器                  ${riwi001}88.  ${color88}Owncast自托管直播平台"
	  echo -e "${riwi001}89.  ${color89}FileCodeBox文件快递                 ${riwi001}90.  ${color90}matrix去中心化聊天协议"
	  echo -e "${riwi001}-------------------------"
	  echo -e "${riwi001}91.  ${color91}gitea私有代码仓库                   ${riwi001}92.  ${color92}FileBrowser文件管理器"
	  echo -e "${riwi001}93.  ${color93}Dufs极简静态文件服务器              ${riwi001}94.  ${color94}Gopeed高速下载工具"
	  echo -e "${riwi001}95.  ${color95}paperless文档管理平台               ${riwi001}96.  ${color96}2FAuth自托管二步验证器"
	  echo -e "${riwi001}97.  ${color97}WireGuard组网(服务端)               ${riwi001}98.  ${color98}WireGuard组网(客户端)"
	  echo -e "${riwi001}99.  ${color99}DSM群晖虚拟机                       ${riwi001}100. ${color100}Syncthing点对点文件同步工具"
	  echo -e "${riwi001}-------------------------"
	  echo -e "${riwi001}101. ${color101}AI视频生成工具                      ${riwi001}102. ${color102}VoceChat多人在线聊天系统"
	  echo -e "${riwi001}103. ${color103}Umami网站统计工具                   ${riwi001}104. ${color104}Stream四层代理转发工具"
	  echo -e "${riwi001}105. ${color105}思源笔记                            ${riwi001}106. ${color106}Drawnix开源白板工具"
	  echo -e "${riwi001}107. ${color107}PanSou网盘搜索                      ${riwi001}108. ${color108}LangBot聊天机器人"
	  echo -e "${riwi001}109. ${color109}ZFile在线网盘                       ${riwi001}110. ${color110}Karakeep书签管理"
	  echo -e "${riwi001}-------------------------"
	  echo -e "${riwi001}111. ${color111}多格式文件转换工具                  ${riwi001}112. ${color112}Lucky大内网穿透工具"
	  echo -e "${riwi001}113. ${color113}Firefox浏览器                       ${riwi001}114. ${color114}OpenClaw机器人管理工具${rw_huang}★${rw_bai}"
	  echo -e "${riwi001}115. ${color115}Hermes机器人管理工具${rw_huang}★${rw_bai}"
	  echo -e "${riwi001}-------------------------"
	  echo -e "${riwi001}第三方应用列表"
  	  echo -e "${riwi001}想要让你的应用出现在这里？查看开发者指南: ${rw_huang}https://dev.riwi.sh/${rw_bai}"
	  ...
	  for f in "$HOME"/apps/*.conf; do
		  ...
		  if echo "$app_numbers" | grep -q "^$base_name$"; then
			  ...
			  echo -e "${riwi001}$base_name${rw_bai} - ${rw_lv}$app_text [已安装]${rw_bai}"
		  else
			  # 如果未安装：正常显示
			  echo -e "${riwi001}$base_name${rw_bai} - $app_text"
		  fi
	  done
	  ...
	  echo -e "${riwi001}-------------------------"
	  echo -e "${riwi001}b.   ${rw_bai}备份全部应用数据                    ${riwi001}r.   ${rw_bai}还原全部应用数据"
	  echo -e "${riwi001}------------------------"
	  echo -e "${riwi001}0.   ${rw_bai}返回主菜单"
	  echo -e "${riwi001}------------------------${rw_bai}"
	  read -e -p "请输入你的选择: " sub_choice
	  ...
	  9|mail)
		...
		while true; do
			...
			if echo "quit" | timeout $timeout telnet smtp.qq.com $port | grep 'Connected'; then
			  echo -e "${rw_lv}端口 $port 当前可用${rw_bai}"
			else
			  echo -e "${rw_hong}端口 $port 当前不可用${rw_bai}"
			fi
			echo ""
	...
	97|wgs)
		...
		sleep 2
		echo
		echo -e "${rw_huang}所有客户端二维码配置: ${rw_bai}"
		docker exec wireguard bash -c 'for i in $(ls /config | grep peer_ | sed "s/peer_//"); do echo "--- $i ---"; /app/show-peer $i; done'
		sleep 2
		echo
		echo -e "${rw_huang}所有客户端配置代码: ${rw_bai}"
		docker exec wireguard sh -c 'for d in /config/peer_*; do echo "# $(basename $d) "; cat $d/*.conf; echo; done'
		sleep 2
		echo -e "${rw_lv}${COUNT}个客户端配置全部输出，使用方法如下：${rw_bai}"
		echo -e "${rw_lv}1. 手机下载wg的APP，扫描上方二维码，可以快速连接网络${rw_bai}"
		echo -e "${rw_lv}2. Windows下载客户端，复制配置代码连接网络。${rw_bai}"
		echo -e "${rw_lv}3. Linux用脚本部署WG客户端，复制配置代码连接网络。${rw_bai}"
		echo -e "${rw_lv}官方客户端下载方式: https://www.wireguard.com/install/${rw_bai}"
		break_end
...
discourse,yunsou,ahhhhfs,nsgame,gying" \
	  ...
	  *)
		...
		if [ -f "$custom_app" ]; then
			...
			echo -e "${rw_hong}错误: 未找到编号为 ${sub_choice} 的应用配置${rw_bai}"
		fi
		  ;;
	esac
	break_end
...
linux_work() {
	...
	while true; do
	  clear
	  send_stats "后台工作区"
	  echo -e "${riwi001}后台工作区${rw_bai}"
	  echo -e "${riwi001}------------------------${rw_bai}"
	  echo ""
	  echo -e "${rw_huang}功能说明:${rw_bai}"
	  echo -e "${riwi001}  提供可以后台常驻运行的工作区，适合执行长时间任务，包括:${rw_bai}"
	  echo -e "${riwi001}  • 支持多个独立的后台工作区${rw_bai}"
	  echo -e "${riwi001}  • 断开SSH连接后任务继续运行${rw_bai}"
	  echo -e "${riwi001}  • 随时查看和恢复后台工作区${rw_bai}"
	  echo ""
	  echo -e "${rw_huang}操作提示:${rw_bai}"
	  echo -e "${riwi001}  • 进入工作区: 选择对应的工作区编号${rw_bai}"
	  echo -e "${riwi001}  • 退出工作区: 按 Ctrl+b 再按 d${rw_bai}"
	  echo -e "${riwi001}  • 切换窗口: 按 Ctrl+b 再按 n${rw_bai}"
	  echo ""
	  echo -e "${riwi001}------------------------"
	  echo "当前已存在的工作区列表"
	  echo -e "${riwi001}------------------------"
	  tmux list-sessions
	  echo -e "${riwi001}------------------------"
	  echo -e "${riwi001}1.   ${rw_bai}1号工作区"
	  echo -e "${riwi001}2.   ${rw_bai}2号工作区"
	  echo -e "${riwi001}3.   ${rw_bai}3号工作区"
	  echo -e "${riwi001}4.   ${rw_bai}4号工作区"
	  echo -e "${riwi001}5.   ${rw_bai}5号工作区"
	  echo -e "${riwi001}6.   ${rw_bai}6号工作区"
	  echo -e "${riwi001}7.   ${rw_bai}7号工作区"
	  echo -e "${riwi001}8.   ${rw_bai}8号工作区"
	  echo -e "${riwi001}9.   ${rw_bai}9号工作区"
	  echo -e "${riwi001}10.  ${rw_bai}10号工作区"
	  echo -e "${riwi001}------------------------"
	  echo -e "${riwi001}21.  ${rw_bai}SSH常驻模式 ${rw_huang}★${rw_bai}"
	  echo -e "${riwi001}22.  ${rw_bai}创建/进入工作区"
	  echo -e "${riwi001}23.  ${rw_bai}注入命令到后台工作区"
	  echo -e "${riwi001}24.  ${rw_bai}删除指定工作区"
	  echo -e "${riwi001}------------------------"
	  echo -e "${riwi001}0.   ${rw_bai}返回主菜单"
	  echo -e "${riwi001}------------------------${rw_bai}"
	  read -e -p "请输入你的选择: " sub_choice
		  ...
		  21)
			while true; do
			  ...
			  if grep -q 'tmux attach-session -t sshd || tmux new-session -s sshd' ~/.bashrc; then
				  ...
				  local tmux_sshd_status="${rw_hui}关闭${rw_bai}"
			  fi
			  send_stats "SSH常驻模式 "
...
linux_Settings() {
	...
	while true; do
	  clear
	  send_stats "系统工具"
	  echo -e "${riwi001}系统工具${rw_bai}"
	  echo -e "${riwi001}------------------------${rw_bai}"
	  echo ""
	  echo -e "${rw_huang}功能说明:${rw_bai}"
	  echo -e "${riwi001}  系统管理和配置工具集合，包括:${rw_bai}"
	  echo -e "${riwi001}  • 系统安全: SSH配置、防火墙、病毒扫描${rw_bai}"
	  echo -e "${riwi001}  • 网络配置: DNS优化、IPV4/IPV6切换、端口管理${rw_bai}"
	  echo -e "${riwi001}  • 系统调优: BBR3、内核参数、虚拟内存${rw_bai}"
	  echo -e "${riwi001}  • 用户管理: 用户创建、权限设置、SSH密钥${rw_bai}"
	  echo -e "${riwi001}  • 系统维护: 备份恢复、定时任务、系统监控${rw_bai}"
	  echo ""
	  echo -e "${rw_huang}提示: 选择对应的数字即可进行相应操作${rw_bai}"
	  echo ""
	  echo -e "${riwi001}------------------------"
	  ...
	  echo -e "${riwi001}101. ${rw_bai}r命令高级用法 ${rw_huang}★${rw_bai}                    ${riwi001}102. ${rw_bai}卸载Riou脚本"
	  echo -e "${riwi001}------------------------"
	  echo -e "${riwi001}0.   ${rw_bai}返回主菜单"
	  echo -e "${riwi001}------------------------${rw_bai}"
	  read -e -p "请输入你的选择: " sub_choice
...
EOF
		  ...
		  while true; do
			  ...
			  echo -e "当前主机名: ${rw_huang}$current_hostname${rw_bai}"
			  echo "------------------------"
		  ...
		  23)
			...
			while true; do
				...
				if [ -f ~/Limiting_Shut_down.sh ]; then
					...
					echo -e "${rw_hui}当前未启用限流关机功能${rw_bai}"
				fi
		  ...
		  66)
			  ...
			  echo "2. 清理系统垃圾文件"
			  echo -e "3. 设置虚拟内存${rw_huang}1G${rw_bai}"
			  echo -e "4. 设置SSH端口号为${rw_huang}5522${rw_bai}"
			  echo -e "5. 启动fail2ban防御SSH暴力破解"
			  echo -e "6. 开放所有端口"
			  echo -e "7. 开启${rw_huang}BBR${rw_bai}加速"
			  echo -e "8. 设置时区到${rw_huang}上海${rw_bai}"
			  echo -e "9. 自动优化DNS地址${rw_huang}海外: 1.1.1.1 8.8.8.8  国内: 223.5.5.5 ${rw_bai}"
		  	  echo -e "10. 设置网络为${rw_huang}ipv4优先${rw_bai}"
			  echo -e "11. 安装基础工具${rw_huang}docker wget sudo tar unzip socat btop nano vim${rw_bai}"
			  echo -e "12. Linux系统内核参数优化${rw_huang}自动根据网络环境调优${rw_bai}"
			  echo "------------------------------------------------"
			  ...
			  case "$choice" in
				[Yy])
				  ...
				  switch_mirror false false
				  linux_update
				  echo -e "[${rw_lv}OK${rw_bai}] 1/12. 更新系统到最新"

				  echo "------------------------------------------------"
				  linux_clean
				  echo -e "[${rw_lv}OK${rw_bai}] 2/12. 清理系统垃圾文件"

				  echo "------------------------------------------------"
				  add_swap 1024
				  echo -e "[${rw_lv}OK${rw_bai}] 3/12. 设置虚拟内存${rw_huang}1G${rw_bai}"

				  echo "------------------------------------------------"
				  new_ssh_port 5522
				  echo -e "[${rw_lv}OK${rw_bai}] 4/12. 设置SSH端口号为${rw_huang}5522${rw_bai}"
				  echo "------------------------------------------------"
				  f2b_install_sshd
				  cd ~
				  f2b_status
				  echo -e "[${rw_lv}OK${rw_bai}] 5/12. 启动fail2ban防御SSH暴力破解"

				  echo "------------------------------------------------"
				  echo -e "[${rw_lv}OK${rw_bai}] 6/12. 开放所有端口"

				  echo "------------------------------------------------"
				  bbr_on
				  echo -e "[${rw_lv}OK${rw_bai}] 7/12. 开启${rw_huang}BBR${rw_bai}加速"

				  echo "------------------------------------------------"
				  set_timedate Asia/Shanghai
				  echo -e "[${rw_lv}OK${rw_bai}] 8/12. 设置时区到${rw_huang}上海${rw_bai}"

				  echo "------------------------------------------------"
				  auto_optimize_dns
				  echo -e "[${rw_lv}OK${rw_bai}] 9/12. 自动优化DNS地址${rw_huang}${rw_bai}"
				  echo "------------------------------------------------"
				  prefer_ipv4
				  echo -e "[${rw_lv}OK${rw_bai}] 10/12. 设置网络为${rw_huang}ipv4优先${rw_bai}}"

				  echo "------------------------------------------------"
				  install_docker
				  install wget sudo tar unzip socat btop nano vim
				  echo -e "[${rw_lv}OK${rw_bai}] 11/12. 安装基础工具${rw_huang}docker wget sudo tar unzip socat btop nano vim${rw_bai}"
				  echo "------------------------------------------------"

				  curl -sS ${gh_proxy}raw.githubusercontent.com/riwi/sh/refs/heads/main/network-optimize.sh | bash
				  echo -e "[${rw_lv}OK${rw_bai}] 12/12. Linux系统内核参数优化"
				  echo -e "${rw_lv}一条龙系统调优已完成${rw_bai}"
		  ...
		  100)
			...
			while true; do
			  ...
			  if grep -q '^ENABLE_STATS="true"' /usr/local/bin/r > /dev/null 2>&1; then
			  	local status_message="${rw_lv}正在采集数据${rw_bai}"
			  elif grep -q '^ENABLE_STATS="false"' /usr/local/bin/r > /dev/null 2>&1; then
			  	local status_message="${rw_hui}采集已关闭${rw_bai}"
			  else
...
run_commands_on_servers() {
	...
	for ((i=0; i<${#SERVER_ARRAY[@]}; i+=5)); do
		...
		echo
		echo -e "${rw_huang}连接到 $name ($hostname)...${rw_bai}"
...
user_permissions_menu() {
  while true; do
    ...
    echo -e "${riwi001}3.   ${rw_bai}查看所有用户 ${rw_huang}★${rw_bai}"
    ...
    case $sub_choice in
      1)
        clear
        send_stats "设置普通用户密码"
        echo -e "${riwi001}设置普通用户密码${rw_bai}"
        echo -e "${riwi001}------------------------${rw_bai}"
        echo ""
        echo -e "${rw_huang}📌 功能说明:${rw_bai}"
        ...
        if ! id "$username" &>/dev/null; then
          echo -e "${rw_hong}用户 $username 不存在${rw_bai}"
          break_end
          continue
        fi
        if [ "$username" = "root" ]; then
          echo -e "${rw_hong}无法修改root用户密码，请使用passwd命令${rw_bai}"
          break_end
          continue
        fi
        echo -e "${rw_huang}请为用户 $username 设置密码:${rw_bai}"
        while true; do
          read -s -p "请输入新密码: " password1
          echo ""
          if [ -z "$password1" ]; then
            echo -e "${rw_hong}密码不能为空${rw_bai}"
            continue
          fi
          read -s -p "请确认新密码: " password2
          echo ""
          if [ "$password1" != "$password2" ]; then
            echo -e "${rw_hong}两次输入的密码不一致，请重新输入${rw_bai}"
            continue
          fi
          break
        done
        echo "$username:$password1" | chpasswd
        echo -e "${rw_lv}✓ 用户 $username 的密码已设置成功${rw_bai}"
        break_end
        ;;
      ...
      2)
        clear
        send_stats "添加sudo权限"
        echo -e "${riwi001}添加sudo权限${rw_bai}"
        echo -e "${riwi001}------------------------${rw_bai}"
        echo ""
        echo -e "${rw_huang}📌 功能说明:${rw_bai}"
        echo -e "${riwi001}  为普通用户添加sudo权限，允许其执行需要root权限的命令${rw_bai}"
        echo -e "${riwi001}  添加后用户可以使用sudo命令执行系统管理操作${rw_bai}"
        echo ""
        echo -e "${rw_huang}📝 使用示例:${rw_bai}"
        echo -e "${riwi001}  输入用户名: owen${rw_bai}"
        echo -e "${riwi001}  确认后用户将可以使用sudo命令${rw_bai}"
        echo ""
        echo -e "${rw_huang}当前具有sudo权限的用户:${rw_bai}"
        ...
        echo -e "${rw_huang}正在为用户 $username 添加sudo权限...${rw_bai}"
        usermod -aG sudo "$username"
        echo -e "${rw_lv}✓ 用户 $username 已成功添加sudo权限${rw_bai}"
        echo -e "${riwi001}  现在用户 $username 可以使用 sudo 命令执行系统管理操作${rw_bai}"
        break_end
        ;;
      ...
      3)
        clear
        send_stats "查看所有用户"
        echo -e "${riwi001}查看所有用户${rw_bai}"
        echo -e "${riwi001}------------------------${rw_bai}"
        echo ""
        echo -e "${rw_huang}📌 功能说明:${rw_bai}"
        echo -e "${riwi001}  显示系统中所有用户的信息，包括用户名、UID、用户类型等${rw_bai}"
        echo ""
        echo -e "${rw_huang}用户类型说明:${rw_bai}"
        echo -e "${riwi001}  • UID 0: root用户（系统管理员）${rw_bai}"
        echo -e "${riwi001}  • UID 1-999: 系统用户（服务账户）${rw_bai}"
        echo -e "${riwi001}  • UID 1000+: 普通用户${rw_bai}"
        echo ""
        echo -e "${rw_huang}系统用户列表 (UID 0-999):${rw_bai}"
      ...
      4)
        clear
        send_stats "创建新用户"
        echo -e "${riwi001}创建新用户${rw_bai}"
        echo -e "${riwi001}------------------------${rw_bai}"
        echo ""
        echo -e "${rw_huang}📌 功能说明:${rw_bai}"
        echo -e "${riwi001}  在服务器上创建一个新的普通用户${rw_bai}"
        echo -e "${riwi001}  新用户创建后可设置密码和添加权限${rw_bai}"
        echo ""
        echo -e "${rw_huang}📝 使用示例:${rw_bai}"
        echo -e "${riwi001}  输入新用户名: newuser${rw_bai}"
        echo -e "${riwi001}  设置密码后即可登录使用${rw_bai}"
        echo ""
        ...
        if id "$username" &>/dev/null; then
          echo -e "${rw_hong}用户 $username 已存在${rw_bai}"
          break_end
          continue
        fi
        echo -e "${rw_huang}正在创建用户 $username ...${rw_bai}"
        useradd -m -s /bin/bash "$username"
        echo -e "${rw_lv}✓ 用户 $username 创建成功${rw_bai}"
        echo -e "${riwi001}  现在可以为该用户设置密码和添加权限${rw_bai}"
        break_end
        ;;
      ...
      5)
        clear
        send_stats "删除用户"
        echo -e "${riwi001}删除用户${rw_bai}"
        echo -e "${riwi001}------------------------${rw_bai}"
        echo ""
        echo -e "${rw_huang}📌 功能说明:${rw_bai}"
        echo -e "${riwi001}  删除服务器上的指定用户${rw_bai}"
        echo -e "${rw_hong}  ⚠️ 警告: 删除用户会同时删除其主目录和邮件 spool${rw_bai}"
        echo ""
        echo -e "${rw_huang}📝 使用示例:${rw_bai}"
        echo -e "${riwi001}  输入要删除的用户名: username${rw_bai}"
        echo -e "${riwi001}  确认后用户及其数据将被删除${rw_bai}"
        echo ""
...
while true; do
	  ...
	  echo -e "${riwi001}------------------------${rw_bai}"
	  echo -e "${riwi001}服务器列表管理${rw_bai}"
	  echo -e "${riwi001}1.  ${rw_bai}添加服务器               ${riwi001}2.  ${rw_bai}删除服务器 "
	  echo -e "${riwi001}3.  ${rw_bai}编辑服务器               ${riwi001}4.  ${rw_bai}备份集群 "
	  echo -e "${riwi001}5.  ${rw_bai}还原集群 "
	  echo -e "${riwi001}------------------------${rw_bai}"
	  echo -e "${riwi001}批量执行任务${rw_bai}"
	  echo -e "${riwi001}11. ${rw_bai}安装Riou脚本             ${riwi001}12. ${rw_bai}更新系统              ${riwi001}13. ${rw_bai}清理系统"
	  echo -e "${riwi001}14. ${rw_bai}安装docker               ${riwi001}15. ${rw_bai}安装BBR3              ${riwi001}16. ${rw_bai}设置1G虚拟内存"
	  echo -e "${riwi001}17. ${rw_bai}设置时区到上海           ${riwi001}18. ${rw_bai}开放所有端口	       ${riwi001}51. ${rw_bai}自定义指令 "
	  echo -e "${riwi001}------------------------${rw_bai}"
	  echo -e "${riwi001}0.  ${rw_bai}返回主菜单"
	  echo -e "${riwi001}------------------------${rw_bai}"
	  read -e -p "请输入你的选择: " sub_choice
...
while true; do
	...
	if [ -z "$sh_v_new" ]; then
		echo -e "${rw_hong}无法获取最新版本信息，请检查网络连接${rw_bai}"
	elif [ "$sh_v" = "$sh_v_new" ]; then
		echo -e "${rw_lv}你已经是最新版本！${rw_huang}v$sh_v${rw_bai}"
		send_stats "脚本已经最新了，无需更新"
	else
		echo "发现新版本！"
		echo -e "当前版本 v$sh_v        最新版本 ${rw_huang}v$sh_v_new${rw_bai}"
	fi
	...
	if [ -n "$existing_cron" ]; then
		echo "------------------------"
		echo -e "${rw_lv}自动更新已开启，每天凌晨2点脚本会自动更新！${rw_bai}"
	fi
		...
		1)
			...
			if curl -sS --max-time 60 --fail -o "$tmp_file" "$download_url" && \
			   ...
			   head -1 "$tmp_file" | grep -q '^#!/bin/bash'; then
				...
				cp -f ~/riwi.sh /usr/local/bin/r > /dev/null 2>&1
				ln -sf /usr/local/bin/r /usr/bin/r > /dev/null 2>&1
				echo -e "${rw_lv}脚本已更新到最新版本！${rw_huang}v$sh_v_new${rw_bai}"
				...
				echo -e "${rw_hong}更新失败！下载出错或文件校验不通过，已恢复原版本${rw_bai}"
				send_stats "脚本更新失败"
			fi
			break_end
			~/riwi.sh
...
switch_git_mirror() {
  while true; do
    ...
    echo -e "${riwi001}║         Git 镜像源切换                 ║${riwi001}"
    echo -e "${riwi001}╚════════════════════════════════════════╝${riwi001}"
    echo ""
    echo -e "${riwi001}1.  ${rw_bai}手动切换源"
    echo -e "${riwi001}2.  ${rw_bai}自动切换最快的源"
    echo -e "${riwi001}3.  ${rw_bai}源测速"
    echo -e "${riwi001}------------------------${rw_bai}"
    echo -e "${riwi001}0.  ${rw_bai}返回上一级"
    echo -e "${riwi001}------------------------${rw_bai}"
...
test_mirror_speed() {
  clear
  echo -e "${riwi001}源测速${rw_bai}"
  echo -e "${riwi001}------------------------${rw_bai}"
  echo ""
  echo -e "${rw_huang}正在测试各个镜像源的速度...${rw_bai}"
  echo ""
  ...
  for mirror in "${mirrors[@]}"; do
    ...
    echo -e "${rw_huang}正在测试 $name...${rw_bai}"
    ...
    if command -v curl &>/dev/null; then
      ...
      if ! [[ $duration =~ ^[0-9]+(\.[0-9]+)?$ ]]; then
        echo -e "  ${rw_hong}✗ $name - 无法连接${rw_bai}"
        continue
      fi
      ...
      if [ $duration_ms -lt 10000 ]; then
        echo -e "  ${rw_lv}✓ $name - ${duration_ms}ms${rw_bai}"
      else
        echo -e "  ${rw_hong}✗ $name - 超时${rw_bai}"
      fi
    else
      echo -e "  ${rw_hong}未安装 curl，无法测速${rw_bai}"
      break
    fi
  done

  echo ""
  echo -e "${rw_lv}测速完成！${rw_bai}"
  echo ""
  read -e -p "按回车继续..."
}
...
manual_switch_mirror() {
  clear
  echo -e "${riwi001}手动切换源${rw_bai}"
  echo -e "${riwi001}------------------------${rw_bai}"
  echo ""
  echo -e "${rw_huang}可用的镜像源:${rw_bai}"
  echo -e "${riwi001}1. ${rw_bai}GitHub 官方源 (https://github.com)"
  echo -e "${riwi001}2. ${rw_bai}GitClone 镜像 (https://gitclone.com)"
  echo -e "${riwi001}3. ${rw_bai}GitHub 加速 (https://mirror.ghproxy.com)"
  echo -e "${riwi001}4. ${rw_bai}FastGit 镜像 (https://hub.fastgit.xyz)"
  echo -e "${riwi001}5. ${rw_bai}自定义源"
  echo ""
  ...
  # 检查是否在 git 仓库中
  if [ ! -d ".git" ]; then
    echo -e "${rw_hong}当前目录不是 git 仓库${rw_bai}"
    echo -e "${rw_huang}正在设置全局 git URL 替换...${rw_bai}"
    git config --global url."${mirror_url}/".insteadOf "https://github.com/"
    echo -e "${rw_lv}已设置全局 URL 替换: ${mirror_url}/ -> https://github.com/${rw_bai}"
  else
    echo -e "${rw_huang}当前 git 仓库的远程配置:${rw_bai}"
    git remote -v
    echo ""
    ...
    if [ $? -ne 0 ]; then
      echo -e "${rw_hong}远程仓库 $remote_name 不存在${rw_bai}"
      read -e -p "按回车继续..."
      return
    fi
    ...
    if echo "$current_url" | grep -q "github.com"; then
      ...
      echo -e "${rw_lv}已将 $remote_name 切换到: $new_url${rw_bai}"
    else
      echo -e "${rw_hong}当前远程仓库不是 GitHub 仓库${rw_bai}"
    fi
  fi
...
auto_switch_fastest_mirror() {
  clear
  echo -e "${riwi001}自动切换最快的源${rw_bai}"
  echo -e "${riwi001}------------------------${rw_bai}"
  echo ""
  echo -e "${rw_huang}正在测试各个镜像源的速度...${rw_bai}"
  echo ""
  ...
  for mirror in "${mirrors[@]}"; do
    ...
    if command -v curl &>/dev/null; then
      ...
      echo -e "  ${rw_hong}未安装 curl，跳过速度测试${rw_bai}"
  ...
  if [ -z "$fastest_mirror" ]; then
    echo -e "${rw_hong}所有镜像源都不可用，使用 GitHub 官方源${rw_bai}"
    fastest_mirror="GitHub官方"
    fastest_url="https://github.com"
  fi

  echo -e "${rw_lv}最快的源: $fastest_mirror (${fastest_time}ms)${rw_bai}"
  echo ""

  # 应用最快的源
  if [ ! -d ".git" ]; then
    echo -e "${rw_huang}当前目录不是 git 仓库，正在设置全局 git URL 替换...${rw_bai}"
    git config --global url."${fastest_url}/".insteadOf "https://github.com/"
    echo -e "${rw_lv}已设置全局 URL 替换: ${fastest_url}/ -> https://github.com/${rw_bai}"
  else
    echo -e "${rw_huang}当前 git 仓库的远程配置:${rw_bai}"
    git remote -v
    echo ""
    ...
    if echo "$current_url" | grep -q "github.com"; then
      ...
      git remote set-url "$remote_name" "$new_url"
      echo -e "${rw_lv}已将 $remote_name 切换到最快源: $new_url${rw_bai}"
    else
      echo -e "${rw_hong}当前远程仓库不是 GitHub 仓库${rw_bai}"
    fi
  fi
  ...
  while true; do
    ...
    case $sub_choice in
      ...
      2)
        clear
        install git
        echo -e "${riwi001}初始化本地仓库${rw_bai}"
        echo -e "目录路径示例: ${rw_hong}./myproject${rw_bai}, ${rw_hong}/home/user/projects/newrepo${rw_bai}"
        echo -e "直接回车使用当前目录"
        echo ""
        echo -e "${rw_huang}提示: 输入完成后按回车继续，输入 0 返回上一级${rw_bai}"
        echo ""
        ...
        git init
        echo -e "${rw_lv}仓库已初始化！${rw_bai}"
        read -e -p "按回车继续..."
        cd ~
        ;;
      3)
        clear
        echo -e "${riwi001}列出本地分支${rw_bai}"
        echo "------------------------"
        if [ -d ".git" ]; then
          git branch -a
        else
        ...
        echo ""
        echo -e "${rw_huang}提示: 按回车继续，输入 0 返回上一级${rw_bai}"
        echo ""
      ...
      4)
        clear
        echo -e "${riwi001}创建新分支${rw_bai}"
        echo -e "分支名称示例: ${rw_hong}main${rw_bai}, ${rw_hong}dev${rw_bai}, ${rw_hong}feature/login${rw_bai}, ${rw_hong}bugfix/issue-123${rw_bai}"
        echo ""
        echo -e "${rw_huang}提示: 输入完成后按回车继续，输入 0 返回上一级${rw_bai}"
        echo ""
        ...
        if [ -d ".git" ]; then
          ...
          echo -e "${rw_lv}分支 $branch_name 已创建并切换！${rw_bai}"
        else
          echo -e "${rw_hong}错误: 当前目录不是git仓库${rw_bai}"
        fi
        read -e -p "按回车继续..."
      ...
      5)
        clear
        echo -e "${riwi001}切换分支${rw_bai}"
        if [ -d ".git" ]; then
          git branch
          echo ""
          echo -e "分支名称示例: ${rw_hong}main${rw_bai}, ${rw_hong}dev${rw_bai}, ${rw_hong}feature/login${rw_bai}"
          echo ""
          echo -e "${rw_huang}提示: 输入完成后按回车继续，输入 0 返回上一级${rw_bai}"
          echo ""
          ...
          echo -e "${rw_lv}已切换到分支 $branch_name${rw_bai}"
        else
          echo -e "${rw_hong}错误: 当前目录不是git仓库${rw_bai}"
        fi
        read -e -p "按回车继续..."
      ...
      6)
        clear
        echo -e "${riwi001}删除分支${rw_bai}"
        if [ -d ".git" ]; then
          git branch
          echo ""
          echo -e "分支名称示例: ${rw_hong}feature/login${rw_bai}, ${rw_hong}bugfix/issue-123${rw_bai}"
          echo ""
          echo -e "${rw_huang}提示: 输入完成后按回车继续，输入 0 返回上一级${rw_bai}"
          echo ""
      ...
      10)
        clear
        echo -e "${riwi001}取消暂存文件${rw_bai}"
        echo -e "文件路径示例: ${rw_hong}README.md${rw_bai}, ${rw_hong}src/main.py${rw_bai}"
        echo -e "取消所有暂存: ${rw_hong}.${rw_bai}"
        echo -e "多个文件用空格分隔: ${rw_hong}file1.txt file2.py${rw_bai}"
        echo ""
        echo -e "${rw_huang}提示: 输入完成后按回车继续，输入 0 返回上一级${rw_bai}"
        echo ""
...
EOF
          ...
          echo -e "                  ${rw_hong}docs: 更新文档${rw_bai}"
          echo -e "                  ${rw_hong}style: 代码格式调整${rw_bai}"
          echo -e "                  ${rw_hong}refactor: 代码重构${rw_bai}"
          ...
          if [ $? -ne 0 ]; then
            ...
            echo -e "${rw_huang}提示: 没有可提交的更改${rw_bai}"
          else
      ...
      12)
        ...
        if [ -d ".git" ]; then
          ...
          echo ""
          echo -e "分支名称示例: ${rw_hong}main${rw_bai}, ${rw_hong}master${rw_bai}, ${rw_hong}dev${rw_bai}, ${rw_hong}feature/login${rw_bai}"
          ...
          if [ $push_result -eq 124 ]; then
            ...
            echo ""
            echo -e "${rw_huang}解决方案:${rw_bai}"
            echo -e "${riwi001}1. ${rw_bai}拉取并合并远程更新 (git pull)"
              ...
              1)
                ...
                if [ $? -eq 0 ]; then
                  ...
                  if [ $? -eq 0 ]; then
                    ...
                    echo -e "${rw_hong}重新推送失败，请手动处理冲突${rw_bai}"
                  fi
                else
                  echo -e "${rw_hong}拉取失败，可能存在冲突需要手动处理${rw_bai}"
                fi
                ;;
              2)
                ...
                if [ $? -eq 0 ]; then
                  ...
                  if [ $? -eq 0 ]; then
                    ...
                    echo -e "${rw_hong}重新推送失败${rw_bai}"
                  fi
                else
                  echo -e "${rw_hong}变基失败，可能存在冲突需要手动处理${rw_bai}"
                fi
                ;;
              3)
                ...
                if [ "$force_confirm" = "y" ] || [ "$force_confirm" = "Y" ]; then
                  ...
                  if [ $? -eq 0 ]; then
                    ...
                    echo -e "${rw_hong}强制推送失败${rw_bai}"
                  fi
                else
                  echo -e "${rw_huang}已取消强制推送${rw_bai}"
                fi
                ;;
              *)
                echo -e "${rw_huang}未选择解决方案${rw_bai}"
                ;;
            esac
          else
            echo -e "${rw_lv}推送成功${rw_bai}"
      ...
      15)
        clear
        echo -e "${riwi001}添加远程仓库${rw_bai}"
        echo "------------------------"
        echo -e "远程仓库名称示例: ${rw_hong}origin${rw_bai} (默认), upstream, myrepo"
        echo ""
        echo -e "${rw_huang}提示: 输入完成后按回车继续，输入 0 返回上一级${rw_bai}"
        echo ""
        if [ -d ".git" ]; then
