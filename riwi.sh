	  ...
	  3)
		...
		case $_pa in
			...
			if [ $_AUDIT_USERS -gt 0 ] && [ $_AUDIT_HAS_PUBKEY -lt $_AUDIT_USERS ]; then
				...
				[ "$_confirm" != "yes" ] && { yellow "已取消"; break_end; continue; }
	...
	case $pk_choice in
	  ...
	  3)
		...
		if [ -f "$_key_path" ]; then
			...
			[[ ! "$_ow" =~ ^[Yy]$ ]] && { yellow "已取消"; break_end; continue; }
		fi
  ...
  while true; do
    ...
    case $sub_choice in
      1)
        ...
        # ── 1. 确保 git 已安装（install 函数内部已自动 sudo）──
        if ! command -v git &>/dev/null; then
            echo -e " ${rw_huang}git 未安装，正在安装...${rw_lv}"
            install git
            # 再次检查
            if ! command -v git &>/dev/null; then
                red "✗ git 安装失败，请手动安装后重试"
                echo -e " ${rw_huang}Debian/Ubuntu: ${rw_lv}sudo apt install git${rw_lv}"
                echo -e " ${rw_huang}CentOS/RHEL:   ${rw_lv}sudo yum install git${rw_lv}"
                break_end
                continue
            fi
            green "✓ git 已安装"
        ...
        echo -e "   ${rw_lv}git@github.com:riwi002/mybox.git${rw_lv}"
        echo ""
        echo -e " ${rw_huang}当前目录: ${rw_lv}$(pwd)${rw_lv}"
        echo -e " ${rw_huang}当前用户: ${rw_lv}$(whoami)${rw_lv}  (uid=$(id -u))${rw_lv}"
        echo ""
        echo -e " ${rw_huang}提示: 直接回车默认克隆工具箱仓库${rw_lv}"
        echo ""

        read -e -p " 请输入仓库URL（回车默认工具箱地址，0返回）: " repo_url
        if [ "$repo_url" = "0" ]; then continue; fi
        [ -z "$repo_url" ] && repo_url="https://github.com/riwi002/mybox.git"

        read -e -p " 请输入目标目录（回车使用仓库名，0返回）: " target_dir
        ...
        # ── 3. 判断目标目录是否可写，决定是否需要 sudo ──
        local _target_path
        if [ -n "$target_dir" ]; then
            _target_path="$target_dir"
        else
            _target_path="$(pwd)/$(basename "$repo_url" .git)"
        fi
        local _target_parent
        _target_parent=$(dirname "$_target_path" 2>/dev/null)
        [ -z "$_target_parent" ] && _target_parent="$(pwd)"

        local _use_sudo=false
        local _sudo_prefix=""

        # 如果当前不是 root 且目标父目录不可写，检查 sudo
        if [ "$EUID" -ne 0 ] && [ ! -w "$_target_parent" ]; then
            # 检查是否有 sudo 权限
            local _can_sudo=false
            if sudo -n true 2>/dev/null; then
                _can_sudo=true
            elif getent group sudo 2>/dev/null | grep -qw "$(whoami)" || \
                 getent group wheel 2>/dev/null | grep -qw "$(whoami)"; then
            ...
            if $_can_sudo; then
                echo ""
                echo -e " ${rw_hong}目标目录 ${_target_parent} 不可写，需要使用 sudo 克隆${rw_lv}"
                echo -e " ${rw_huang}需要验证 sudo 密码:${rw_lv}"
                ...
                red "目标目录 ${_target_parent} 不可写，且当前用户无 sudo 权限"
                echo -e " ${rw_huang}请换到有权限的目录再试，或联系管理员${rw_lv}"
                break_end
                continue
            fi
        fi
        ...
        # ── 4. 执行克隆 ──
        echo ""
        echo -e " ${rw_huang}正在克隆...${rw_lv}"
        echo -e " ${rw_lv}URL: ${repo_url}${rw_lv}"
        if [ -n "$target_dir" ]; then
            echo -e " ${rw_lv}目标: ${target_dir}${rw_lv}"
        else
            echo -e " ${rw_lv}目标: $(basename "$repo_url" .git)${rw_lv}"
        fi
        if $_use_sudo; then
            echo -e " ${rw_lv}模式: sudo 克隆${rw_lv}"
        else
            echo -e " ${rw_lv}模式: 普通克隆${rw_lv}"
        fi
        echo ""

        local _clone_rc=0
        if $_use_sudo; then
            # sudo 克隆：sudo git clone，然后把属主改回当前用户
            local _cur_user
            _cur_user=$(whoami)
            if [ -z "$target_dir" ]; then
                $_sudo_prefix git clone "$repo_url"
                _clone_rc=$?
                local _repo_name
                _repo_name=$(basename "$repo_url" .git)
                if [ -d "$_repo_name" ]; then
                    $_sudo_prefix chown -R "$_cur_user":"$(id -gn "$_cur_user")" "$_repo_name" 2>/dev/null
                    green "✓ 已将 ${_repo_name} 属主改为 $_cur_user"
                fi
            else
                $_sudo_prefix git clone "$repo_url" "$target_dir"
                _clone_rc=$?
                if [ -d "$target_dir" ]; then
                    $_sudo_prefix chown -R "$_cur_user":"$(id -gn "$_cur_user")" "$target_dir" 2>/dev/null
                    green "✓ 已将 ${target_dir} 属主改为 $_cur_user"
                fi
            fi
        else
            if [ -z "$target_dir" ]; then
        ...
        if [ $_clone_rc -eq 0 ]; then
            ...
            red "✗ 克隆失败 (退出码: $_clone_rc)"
            echo -e " ${rw_huang}常见原因:${rw_lv}"
            echo -e "   - 仓库 URL 错误或不存在"
            echo -e "   - 网络不通或 DNS 解析失败"
            echo -e "   - 目标目录已存在且非空"
            echo -e "   - 权限不足（sudo 模式可解决）"
        fi
        echo ""
        read -e -p " 按回车继续..."
        ;;
