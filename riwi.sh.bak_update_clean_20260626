...
github_manager() {
  while true; do
    ...
    echo ""
    echo -e " ${rw_cheng}──── 高级 ────${rw_lv}"
    echo -e "  ${rw_huang}13${rw_lv} 切换 Git 镜像源      ${rw_huang}14${rw_lv} GitHooks 部署"
    echo -e "${rw_cheng}────────────────────────────────────────${rw_lv}"
    echo -e "  ${rw_huang}0${rw_lv}  返回主菜单"
    echo -e "${rw_cheng}────────────────────────────────────────${rw_lv}"
    ...
    case $sub_choice in
      1)
        ...
        if [ -z "$_remote_chk" ]; then
            ...
            if [[ ! "$_push" =~ ^[Nn]$ ]]; then
                ...
                _push_out=$(git -c core.askpass= push "$_remote_chk" "$_br" 2>&1)
                _push_rc=$?
                if [ $_push_rc -eq 0 ]; then
                    ...
                    if [[ "$_retry" =~ ^[Yy]$ ]]; then
                        git pull --rebase "$_remote_chk" "$_br" 2>&1 | head -10
                        _push_out=$(git push "$_remote_chk" "$_br" 2>&1)
                        _push_rc=$?
                        if [ $_push_rc -eq 0 ]; then
      ...
      2)
        ...
        echo ""
        echo -e " ${rw_cheng}── 执行拉取 ──${rw_lv}"
        git pull "$_rem" "${_br:-main}" 2>&1
        if [ $? -eq 0 ]; then
            echo ""
      ...
      8)
        ...
        echo ""
        echo -e " ${rw_huang}回退方式:${rw_lv}"
        echo -e "   ${rw_huang}1${rw_lv} 软回退（保留更改在暂存区）"
        echo -e "   ${rw_huang}2${rw_lv} 硬回退（${rw_hong}丢弃所有更改！${rw_lv}）"
        echo -e "   ${rw_huang}0${rw_lv} 取消"
        read -e -p " 请选择: " _reset_choice < /dev/tty
        case $_reset_choice in
            1) git reset --soft "$_commit_id" 2>&1 && echo -e " ${rw_lv}✓ 已软回退到 $_commit_id${rw_lv}" ;;
            2)
                read -e -p " $(echo -e "${rw_hong}确认硬回退？(输入 YES): ${rw_lv}")" _confirm < /dev/tty
                [ "$_confirm" = "YES" ] && git reset --hard "$_commit_id" 2>&1 && \
                    echo -e " ${rw_lv}✓ 已硬回退到 $_commit_id${rw_lv}" || echo -e " ${rw_huang}已取消${rw_lv}"
                ;;
            0) continue ;;
        esac
        echo ""
